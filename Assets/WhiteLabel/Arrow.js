if (document.querySelector('link[href^="/css/exam-result/result.css"]') == null) {
    function handleToggles (){
      // ShowHints
      let showHints = $('select[data-name="ShowHints"]').val();
      // ShowHints = yes or missing
      if (showHints == null || showHints.toUpperCase() !== 'NO') {
        $('input.checkMode').attr('checked', true);
      }
      // ShowHints = no 
      else {
        $('input.checkMode').removeAttr('checked');
        $(".hint, .hint-icon, .hintLink, .hiddenItem, .knowledge, .know-icon, .knowledgeLink, .moreKnowledge ").hide();
      }
      // ShowToggle
      let showToggle = $('select[data-name="ShowToggle"]').val();
      //ShowToggle = no
      if (showToggle != null && showToggle.toUpperCase() == 'NO') {
      ...
      // ShowToggle = yes or missing
        ...
        $('body').on('click', 'input.checkMode', function () {
          ...
          if ($(this).is(':checked')) {
            ...
            $(".hint, .hint-icon, .hintLink, .hiddenItem, .knowledge, .know-icon, .knowledgeLink, .moreKnowledge ").show();
            // Is this the editor?
    ...
    setTimeout(()=>{handleToggles()}, 750);
    
    // Fallback next button toggle handling
    ...
    // Arrow customization
    const disclaimerFeedback = $('#lab-disclaimer-custom-url'),
        endOfLabFeedback = $('#lab-feedback-custom-url'),
        submitSupportRequest = $('#helpTab .supportIcon a'),
        link = 'mailto:remotelab.ecs.de@arrow.com';
  
    if (disclaimerFeedback.length) { disclaimerFeedback.attr('href', link); }
    if (endOfLabFeedback.length) { endOfLabFeedback.attr('href', link); }
    if (submitSupportRequest.length) { 
        submitSupportRequest.attr('href', link); 
        submitSupportRequest.unbind("click").bind("click");
    }  
  } else {
